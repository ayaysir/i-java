...
<body class=container>
	...
	<div class=container id=wrapper>
		...
		href="${pageContext.servletContext.contextPath}/read.qna?seq=${ item.seq }&page=${ (param.page != null) ? param.page : 1  }">
		${ item.title } 
		<span class=text-success style="font-size: 10px">${ replyCount[st.index] != 0 ? [replyCount[st.index]] : "" }</span>
		...
		<c:if test="${ item.likeCount != 0 }">♥${ item.likeCount }</c:if>
						...
						<td class="centeral" id="writercol">${levelMarkup1[st.index]} ${ item.nickname }</td>
			...
			<tr>
				...
				<td style="width: 33%;">
					<ul class="pagination pagination-sm">		
						...
						<c:forEach var="item" items="${ navi }">
							...
							<a class="page-link" href="list.qna?page=${item.link}">${item.symbol}</a>
				...
				<td style="width: 33%;">
					<div class="btn-group" role="group" aria-label="Basic example">
						<c:if test="${ sessionScope.cluLevel >= 108 }">
							<button type="button" class="btn btn-secondary">수정</button>
							<button type="button" class="btn btn-secondary">삭제</button>
						</c:if>
						<button type="button" class="btn btn-primary" id=writeBtn>질문하기</button>
					</div>
				</td>
			</tr>
		</table>
